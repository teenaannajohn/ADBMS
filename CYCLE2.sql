...
WHERE NOT EXISTS ( SELECT RESERVS.BID FROM RESERVS  WHERE RESERVS.SID = SAILORS.SID  AND RESERVS.BID=BOATS.BID));
...
SELECT QTY_SOLID,ITEM_NAME,I.PRICE,(QTY_SOLID*PRICE) AS FINAL_AMOUNT FROM SALE S,CUSTOMER C,ITEM I WHERE 
S.CUST_ID=C.CUST_ID AND I.ITEM_ID=S.ITEM_ID AND C.CUST_NAME='REKHA';
